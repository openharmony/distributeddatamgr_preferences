...
public:
    ...
    template<typename O> sptr(const sptr<O> &other);
...
template<typename T> inline void sptr<T>::ForceSetRefPtr(T *other)
{
    refs_ = other;
}
...
template<typename T> inline sptr<T>::sptr(T *other)
{
    refs_ = other;
}

template<typename T> inline sptr<T>::sptr(const sptr<T> &other)
{
...
template<typename T> sptr<T>::sptr(sptr<T> &&other)
{
    refs_ = other.GetRefPtr();
    other.ForceSetRefPtr(nullptr);
}

template<typename T> sptr<T> &sptr<T>::operator=(sptr<T> &&other)
{
    if (refs_ != nullptr) {
...
template<typename T> template<typename O> sptr<T>::sptr(const sptr<O> &other) : refs_(other.GetRefPtr())
{
    if (refs_ != nullptr) {
        refs_->IncStrongRef(this);
    }
...
template<typename T> inline sptr<T> &sptr<T>::operator=(T *other)
{
    if (other != nullptr) {
...
template<typename T> inline sptr<T> &sptr<T>::operator=(const sptr<T> &other)
{
...
template<typename T> inline sptr<T> &sptr<T>::operator=(const wptr<T> &other)
{
...
template<typename T> template<typename O> sptr<T> &sptr<T>::operator=(const sptr<O> &other)
{
    T *otherRef(other.GetRefPtr());
    if (otherRef != nullptr) {
        otherRef->IncStrongRef(this);
...
template<typename T> inline bool sptr<T>::operator==(const T *other) const
{
    return other == refs_;
}

template<typename T> inline bool sptr<T>::operator==(const wptr<T> &other) const
{
    return refs_ == other.GetRefPtr();
}

template<typename T> inline bool sptr<T>::operator==(const sptr<T> &other) const
{
    return refs_ == other.GetRefPtr();
}

template<typename T> void sptr<T>::clear()
{
    if (refs_) {
        refs_->DecStrongRef(this);
        refs_ = 0;
...
public:
    ...
    template<typename O> wptr(const wptr<O> &other);
...
template<typename T> inline T *wptr<T>::GetRefPtr() const
{
    return (refs_ != nullptr) ? reinterpret_cast<T *>(refs_->GetRefPtr()) : nullptr;
}

template<typename T> wptr<T>::wptr()
{
    refs_ = nullptr;
}

template<typename T> wptr<T>::wptr(T *other)
{
    if (other != nullptr) {
...
template<typename T> wptr<T>::wptr(const wptr<T> &other)
{
    refs_ = other.refs_;
    if (refs_ != nullptr) {
        refs_->IncWeakRefCount(this);
...
template<typename T> wptr<T>::wptr(const sptr<T> &other)
{
...
template<typename T> template<typename O> wptr<T>::wptr(const wptr<O> &other)
{
    refs_ = other.refs_;
    if (refs_ != nullptr) {
        refs_->IncWeakRefCount(this);
...
template<typename T> template<typename O> wptr<T>::wptr(const sptr<O> &other)
{
    if (other.GetRefPtr() != nullptr) {
        refs_ = other->CreateWeakRef(other.GetRefPtr());
        if (refs_ != nullptr) {
...
template<typename T> wptr<T> &wptr<T>::operator=(T *other)
{
...
template<typename T> template<typename O> wptr<T> &wptr<T>::operator=(O *other)
{
...
template<typename T> inline wptr<T> &wptr<T>::operator=(const wptr<T> &other)
{
...
template<typename T> inline wptr<T> &wptr<T>::operator=(const sptr<T> &other)
{
    WeakRefCounter *newWeakRef = nullptr;
    if (other.GetRefPtr() != nullptr) {
...
template<typename T> template<typename O> wptr<T> &wptr<T>::operator=(const wptr<O> &other)
{
    if (other.refs_ != nullptr) {
        other.refs_->IncWeakRefCount(this);
    }
...
template<typename T> template<typename O> wptr<T> &wptr<T>::operator=(const sptr<O> &other)
{
    WeakRefCounter *newWeakRef = nullptr;
    if (other.GetRefPtr() != nullptr) {
        newWeakRef = other->CreateWeakRef(other->GetRefPtr());
...
template<typename T> inline bool wptr<T>::operator==(const T *other) const
{
    return GetRefPtr() == other;
}

template<typename T> inline bool wptr<T>::operator==(const wptr<T> &other) const
{
    return GetRefPtr() == other.GetRefPtr();
}

template<typename T> inline bool wptr<T>::operator==(const sptr<T> &other) const
{
    return GetRefPtr() == other.GetRefPtr();
}

template<typename T> inline const sptr<T> wptr<T>::promote() const
{
    return sptr<T>(refs_, true);
}

template<typename T> inline wptr<T>::~wptr()
{
    if (refs_ != nullptr) {
        refs_->DecWeakRefCount(this);
    }
