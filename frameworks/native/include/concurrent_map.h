...
public:
    ...
    ConcurrentMap(std::map<_Key, _Tp> &&other) noexcept
    {
        operator=(std::move(other));
    }

    ConcurrentMap &operator=(std::map<_Key, _Tp> &&other) noexcept
    {
        if (&entries_ == &other) {
            return *this;
        }
        std::unique_lock<decltype(mutex_)> lock(mutex_);
        entries_ = std::move(other);
        return *this;
    }
    ...
    size_type EraseIf(const key_type &key,
        const std::function<bool(const key_type &key, mapped_type &value)> &action) noexcept
    {
        if (action == nullptr) {
            return 0;
        }
        std::unique_lock<decltype(mutex_)> lock(mutex_);
        auto it = entries_.find(key);
        if (it == entries_.end() || !action((*it).first, (*it).second)) {
            return 0;
        }
        return entries_.erase(key);
    }
    ...
    void DoActionWhenClone(const std::function<void(const std::map<_Key, _Tp> &)> &action)
    {
        if (action == nullptr) {
            return;
        }
