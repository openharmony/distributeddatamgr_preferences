...
using TaihePreferences = ohos::data::preferences::Preferences;
using TaiheOptions = ohos::data::preferences::Options;
...
static TaihePreferences defaultPreferences = make_holder<PreferencesProxy, TaihePreferences>();
...
std::shared_ptr<JSError> ParseOptions(TaiheOptions const& options, PreferencesInfo &preferencesInfo)
{
...
TaihePreferences GetPreferences(uintptr_t context, PreferencesInfo &info)
{
    auto err = ParseContext(context, info);
    if (err != nullptr) {
        SetBusinessError(err);
        return make_holder<PreferencesProxy, TaihePreferences>();
    }
    ...
    return make_holder<PreferencesProxy, TaihePreferences>(preferences);
}

TaihePreferences GetPreferencesSync(uintptr_t context, TaiheOptions const& options)
{
...
TaihePreferences GetPreferencesSyncByName(uintptr_t context, string_view name)
{
    PreferencesInfo preferencesInfo = {
...
void DeletePreferencesSync(uintptr_t context, TaiheOptions const& options)
{
    PreferencesInfo preferencesInfo;
    auto errCode = ParseOptions(options, preferencesInfo);
    if (errCode != nullptr) {
...
void RemovePreferencesFromCacheSyncByOptions(uintptr_t context, TaiheOptions const& options)
{
    PreferencesInfo preferencesInfo;
    auto errCode = ParseOptions(options, preferencesInfo);
    if (errCode != nullptr) {
